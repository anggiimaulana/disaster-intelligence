...
export default function KesiapsiagaanForm() {
    ...
    const [thumbnailMode, setThumbnailMode] = useState<'upload' | 'icon'>(item?.icon ? 'icon' : 'upload');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEdit && item) {
            ...
            router.post(`/cms/kesiapsiagaan/${item.slug}`, formData, {
                preserveScroll: true,
                onFinish: () => setSubmitting(false),
            });
        } else {
            router.post('/cms/kesiapsiagaan', formData, {
                preserveScroll: true,
                onFinish: () => setSubmitting(false),
            });
        }
    ...
    return (
        <>
            <Head title={isEdit ? 'Edit Kesiapsiagaan' : 'Tambah Kesiapsiagaan'} />

            <div className="mb-5">
                <a href="/cms/kesiapsiagaan" className="mb-4 inline-flex items-center gap-1 text-sm font-medium text-blue-600 hover:text-blue-700">
                    <ArrowLeft className="h-4 w-4" /> Kembali
                </a>
                <h1 className="text-xl font-bold text-slate-900">{isEdit ? 'Edit Kesiapsiagaan' : 'Tambah Kesiapsiagaan Baru'}</h1>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5 pb-20">
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 className="mb-4 text-base font-bold text-slate-900">Informasi</h2>
                    <div className="space-y-6">
                        <div>
                            <label className="mb-1.5 block text-sm font-medium text-slate-700">Judul *</label>
                            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none" placeholder="Masukkan judul" />
                            {errors?.title && <p className="mt-1 text-xs text-red-600">{errors.title}</p>}
                        </div>
                        <div>
                            <label className="mb-1.5 block text-sm font-medium text-slate-700">Konten *</label>
                            <RichTextEditor value={content} onChange={setContent} placeholder="Tulis konten kesiapsiagaan yang lengkap di sini..." minHeight="300px" />
                        ...
                        <div>
                            ...
                            {thumbnailMode === 'upload' ? (
                                <div onClick={() => fileInputRef.current?.click()} className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-200 p-6 hover:border-blue-400 hover:bg-blue-50/50 transition-colors">
                                    {thumbnailPreview || (item?.thumbnail && !removeThumbnail) ? (
                                        <div className="relative">
                                            <img src={thumbnailPreview || `/storage/${item?.thumbnail}`} alt="Preview" className="max-h-48 rounded-lg object-cover shadow-sm" />
                                            <button type="button" onClick={(e) => { e.stopPropagation(); removeFile(); }} className="absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white shadow hover:bg-red-600"><X className="h-3.5 w-3.5" /></button>
                                        </div>
                                    ) : (
                                        <>
                ...
                <SeoAnalyzer
                    ...
                    fallbackTitle={title}
                    previewSlug={item?.slug || 'preview-slug'}
                    sectionLabel="kesiapsiagaan"
                />

                <div className="flex items-center justify-end gap-3 pt-4">
                    <a href="/cms/kesiapsiagaan" className="rounded-lg border border-slate-200 px-6 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">Batal</a>
